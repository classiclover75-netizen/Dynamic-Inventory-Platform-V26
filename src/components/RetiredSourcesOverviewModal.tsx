...
export function RetiredSourcesOverviewModal({
  ...
  const sourceColumns = useMemo(() => {
    let cols = columns.filter((c: any) => c.key !== 'sr');
    if (!showSaleColumns) {
          ...
          <table className="w-max table-fixed text-sm border-collapse" style={{ width: totalWidth + 'px' }}>
            <thead className="bg-gray-100 shadow-sm">
              <tr>
                <th className={getHeaderCls('__retired_source', "p-2 border text-left bg-purple-50 text-purple-800 relative")} style={getHeaderSty('__retired_source', getColWidth('__retired_source'))}>
                  ...
                  <OverviewColumnResizeHandle colId="__retired_source" width={getColWidth('__retired_source')} startResize={startResize} resetCol={resetCol} columnName="Retired Source" />
                </th>
            ...
            <tbody>
              {filteredRows.map((row, i) => {
                ...
                return (
                <tr key={`${row._originalRowId}-${row._retiredSourceName}-${i}`} className={rowColorStyle ? "" : "hover:bg-gray-50"}>
                  <td className={getBodyCls('__retired_source', sourceCls)} style={getBodySty('__retired_source', getColWidth('__retired_source'), rowColorStyle || undefined)}>
                    <div className="flex items-center gap-1">
                      {(() => {
                    ...
                    <div className={qtyCls}>Qty: {row._retiredQty}</div>
                  </td>
